...
function Get-ServerNetworkInventorySafe {
  ...
  if ($adapterConfigs.Count -eq 0) {
    return [pscustomobject]@{
      ...
      DNSSummary     = 'Não foi possível consultar DNS das interfaces.'
      Interfaces     = @()
    }
  }
  ...
  $interfaces = foreach ($adapter in $adapterConfigs) {
    ...
    $dnsRegistration = 'Unknown'
    try {
      if ($adapter.FullDNSRegistrationEnabled -eq $true) { $dnsRegistration = 'Enabled' }
      elseif ($adapter.FullDNSRegistrationEnabled -eq $false) { $dnsRegistration = 'Disabled' }
    } catch {}

    [pscustomobject]@{
      Name            = $adapterName
      Addresses       = if ($formattedAddresses.Count -gt 0) { $formattedAddresses -join ', ' } else { '(sem endereço IP válido)' }
      DNSServers      = if ($dnsServers.Count -gt 0) { $dnsServers -join ', ' } else { '(nenhum DNS configurado)' }
      DNSRegistration = $dnsRegistration
    }
  }
  ...
  $dnsSummary = @(
    $interfaces | ForEach-Object { "{0}: {1}" -f $_.Name, $_.DNSServers }
  ) -join ' | '
...
function Convert-ReplicationPartnerDisplayName {
  param($Partner)

  if ($null -eq $Partner) { return 'Unknown' }

  try {
    $nameValue = Get-ObjectPropertyValueSafe -InputObject $Partner -Names @('HostName','Name')
    if ($nameValue) {
      $candidate = ([string]$nameValue).Trim()
      if ($candidate -and $candidate -notmatch '^CN=') { return $candidate }
    }
  } catch {}

  $text = ([string]$Partner).Trim()
  if ([string]::IsNullOrWhiteSpace($text)) { return 'Unknown' }

  # Exemplo comum: CN=NTDS Settings,CN=DC01,CN=Servers,CN=Site01,...
  if ($text -match '(?i)CN=NTDS Settings,CN=([^,]+),CN=Servers') {
    return $Matches[1]
  }
...
function Get-ReplicationDetailsSafe {
  param([Parameter(Mandatory)][string]$ComputerName)

  $rows = @()
  $queryError = ''

  if (-not (Get-Command Get-ADReplicationPartnerMetadata -ErrorAction SilentlyContinue)) {
    try { Import-Module ActiveDirectory -ErrorAction Stop } catch {}
  }

  if (Get-Command Get-ADReplicationPartnerMetadata -ErrorAction SilentlyContinue) {
    try {
      $metadata = @(
        Get-ADReplicationPartnerMetadata -Target $ComputerName -Scope Server -PartnerType Inbound -Partition * -ErrorAction Stop
      )

      foreach ($item in $metadata) {
        $partnerRaw = Get-ObjectPropertyValueSafe -InputObject $item -Names @('Partner')
        $partitionRaw = Get-ObjectPropertyValueSafe -InputObject $item -Names @('Partition','NamingContext')
        $attemptRaw = Get-ObjectPropertyValueSafe -InputObject $item -Names @('LastReplicationAttempt')
        $successRaw = Get-ObjectPropertyValueSafe -InputObject $item -Names @('LastReplicationSuccess')
        $resultRaw = Get-ObjectPropertyValueSafe -InputObject $item -Names @('LastReplicationResult','LastResult')
        $failureRaw = Get-ObjectPropertyValueSafe -InputObject $item -Names @('ConsecutiveReplicationFailures','ConsecutiveFailureCount','FailureCount')

        $lastAttemptDate = Convert-ToDateTimeSafe -Value $attemptRaw
        $lastSuccessDate = Convert-ToDateTimeSafe -Value $successRaw

        $resultCode = 'Unknown'
        $resultMessage = 'Resultado não disponível'
        $resultNumber = $null
        if ($null -ne $resultRaw -and -not [string]::IsNullOrWhiteSpace([string]$resultRaw)) {
          $resultCode = [string]$resultRaw
          $tmpResult = 0
          if ([int]::TryParse([string]$resultRaw, [ref]$tmpResult)) {
            $resultNumber = $tmpResult
            if ($tmpResult -eq 0) {
              $resultMessage = 'Sucesso'
            } else {
              try {
                $msg = (New-Object System.ComponentModel.Win32Exception -ArgumentList $tmpResult).Message
                $resultMessage = if ($msg) { $msg } else { 'Erro de replicação' }
              } catch { $resultMessage = 'Erro de replicação' }
            }
          }
        }

        $failureCount = if ($null -ne $failureRaw -and -not [string]::IsNullOrWhiteSpace([string]$failureRaw)) {
          [string]$failureRaw
        } elseif ($null -ne $resultNumber -and $resultNumber -eq 0) {
          '0'
        } else {
          'Unknown'
        }
        ...
        $rows += [pscustomobject]@{
          Partner             = Convert-ReplicationPartnerDisplayName -Partner $partnerRaw
          Partition           = if ($partitionRaw) { [string]$partitionRaw } else { 'Unknown' }
          LastAttempt         = if ($lastAttemptDate) { $lastAttemptDate.ToString('dd/MM/yyyy HH:mm:ss') } else { 'Unknown' }
          LastSuccess         = if ($lastSuccessDate) { $lastSuccessDate.ToString('dd/MM/yyyy HH:mm:ss') } else { 'Unknown' }
          ResultCode          = $resultCode
          ResultMessage       = $resultMessage
          ConsecutiveFailures = $failureCount
          IsSuccess           = ($null -ne $resultNumber -and $resultNumber -eq 0)
        }
      }
    } catch {
    ...
    $queryError = 'Módulo ActiveDirectory / Get-ADReplicationPartnerMetadata não está disponível.'
  }

  $failureCountTotal = @($rows | Where-Object { -not $_.IsSuccess }).Count
  $summary = if ($rows.Count -gt 0) {
    "Relacionamentos: $($rows.Count); com erro/indeterminado: $failureCountTotal"
  } elseif ($queryError) {
    "Não foi possível coletar: $queryError"
  } else {
    'Nenhum relacionamento inbound retornado.'
  }
...
function Invoke-ExternalCommandCaptureSafe {
  param(
  ...
  try {
    ...
    if (-not $finished) {
      try { $proc.Kill() | Out-Null } catch {}
      return [pscustomobject]@{ Success=$false; ExitCode=-1; Output='Timeout ao executar comando.' }
    }

    $output = ($proc.StandardOutput.ReadToEnd() + $proc.StandardError.ReadToEnd()).Trim()
    return [pscustomobject]@{ Success=($proc.ExitCode -eq 0); ExitCode=$proc.ExitCode; Output=$output }
  } catch {
...
function Get-TimeSyncInfoSafe {
  param(
    [Parameter(Mandatory)][string]$ComputerName,
    [bool]$IsPdcEmulator = $false
  )

  $serviceStatus = Get-ServiceStatusSafe -ComputerName $ComputerName -ServiceName 'W32Time'
  $source = 'Unknown'
  $lastSync = 'Unknown'
  $stratum = 'Unknown'
  $serverTime = 'Unknown'
  ...
  # Hora local reportada pelo servidor remoto.
  $localTime = @(Get-RemoteCimDataSafe -ComputerName $ComputerName -ClassName 'Win32_LocalTime') | Select-Object -First 1
  if ($localTime) {
    try {
      $serverTime = ('{0:00}/{1:00}/{2:0000} {3:00}:{4:00}:{5:00}' -f [int]$localTime.Day, [int]$localTime.Month, [int]$localTime.Year, [int]$localTime.Hour, [int]$localTime.Minute, [int]$localTime.Second)
    } catch {}
  }

  $w32tm = Join-Path $env:SystemRoot 'System32\w32tm.exe'
  if (-not (Test-Path -LiteralPath $w32tm)) {
    $cmd = Get-Command w32tm.exe -ErrorAction SilentlyContinue
    if ($cmd) { $w32tm = $cmd.Source }
  }

  if ($w32tm -and (Test-Path -LiteralPath $w32tm)) {
    $sourceResult = Invoke-ExternalCommandCaptureSafe -FilePath $w32tm -Arguments "/query /computer:$ComputerName /source" -TimeoutSeconds 20
    if ($sourceResult.Success -and $sourceResult.Output) {
      $sourceLines = @($sourceResult.Output -split '\r?\n' | ForEach-Object { $_.Trim() } | Where-Object { $_ })
      if ($sourceLines.Count -gt 0) { $source = $sourceLines[0] }
    } elseif ($sourceResult.Output) {
      $queryDetail = $sourceResult.Output
    }

    $statusResult = Invoke-ExternalCommandCaptureSafe -FilePath $w32tm -Arguments "/query /computer:$ComputerName /status" -TimeoutSeconds 20
    if ($statusResult.Success -and $statusResult.Output) {
      $statusLines = @($statusResult.Output -split '\r?\n' | ForEach-Object { $_.Trim() } | Where-Object { $_ })

      # O rótulo do w32tm é localizado. Para o último sincronismo, procura a linha
      # que contém uma data/hora; normalmente é a única data completa no /status.
      $dateLine = $statusLines | Where-Object {
        $_ -match '\d{1,2}[\/\.\-]\d{1,2}[\/\.\-]\d{2,4}.*\d{1,2}:\d{2}' -or
        $_ -match '\d{4}[\/\.\-]\d{1,2}[\/\.\-]\d{1,2}.*\d{1,2}:\d{2}'
      } | Select-Object -First 1
      if ($dateLine) {
        if ($dateLine -match '^[^:]+:\s*(.+)$') { $lastSync = $Matches[1].Trim() }
        else { $lastSync = $dateLine }
      }

      $stratumLine = $statusLines | Where-Object { $_ -match '(?i)stratum|estrato' } | Select-Object -First 1
      if ($stratumLine -and $stratumLine -match ':\s*(\d+)') { $stratum = $Matches[1] }
    } elseif (-not $queryDetail -and $statusResult.Output) {
      $queryDetail = $statusResult.Output
    }
  } else {
    $queryDetail = 'w32tm.exe não foi encontrado.'
  }

  $health = 'OK'
  $healthDetail = 'Fonte de horário consultada com sucesso.'
  if ($serviceStatus -ne 'Running') {
    $health = 'Aviso'
    $healthDetail = "Serviço W32Time em estado '$serviceStatus'."
  } elseif ($source -eq 'Unknown') {
    $health = 'Aviso'
    if ($queryDetail) { $healthDetail = "Falha na consulta W32Time: $queryDetail" }
    else { $healthDetail = 'Não foi possível identificar a fonte de horário.' }
  } elseif ($source -match '(?i)Local CMOS|CMOS Clock|Free-running|Free Running|Rel[oó]gio.*CMOS') {
    $health = 'Aviso'
    $healthDetail = "Fonte local detectada: $source"
  }

  [pscustomobject]@{
    IsPdcEmulator = $IsPdcEmulator
...
$PdcRoleOwners        = @(
  $FSMORoles | Where-Object { $_.Role -eq 'PDC Emulator' -and $_.Server -and $_.Server -ne 'Unknown' } |
    ForEach-Object { [string]$_.Server } | Sort-Object -Unique
)
...
$ResultsArr = foreach ($dcFqdn in $DCServers) {
  ...
  # Interfaces, endereçamento e DNS configurado nas placas
  $networkInfo = Get-ServerNetworkInventorySafe -ComputerName $target

  # Replicação detalhada (parceiros inbound / todas as partições)
  $replicationInfo = Get-ReplicationDetailsSafe -ComputerName $target

  # Identifica se este DC possui o papel PDC Emulator de algum domínio da floresta.
  $isPdcEmulator = $false
  foreach ($pdcServer in $PdcRoleOwners) {
    $pdcShort = ([string]$pdcServer -split '\.')[0]
    if (([string]$pdcServer -ieq $dcFqdn) -or ($pdcShort -ieq $short)) {
      $isPdcEmulator = $true
      break
    }
  }
  ...
  [pscustomobject]@{
    ...
    NetworkDNSServers     = $networkInfo.DNSSummary
    NetworkInterfaces     = $networkInfo.Interfaces
    ReplicationRelationshipCount = $replicationInfo.RelationshipCount
    ReplicationFailureCount      = $replicationInfo.FailureCount
    ReplicationSummary           = $replicationInfo.Summary
    ReplicationDetails           = $replicationInfo.Details
    IsPdcEmulator                = $timeInfo.IsPdcEmulator
    W32TimeService               = $timeInfo.ServiceStatus
    ServerLocalTime              = $timeInfo.ServerTime
    TimeSource                   = $timeInfo.Source
    LastTimeSync                 = $timeInfo.LastSync
...
<style>
  * { box-sizing: border-box; }
  ...
  .network-table .network-dns {
    min-width: 280px;
    font-family: Consolas, "Courier New", monospace;
  }

  .network-table .network-dns-registration {
    min-width: 130px;
    text-align: center;
    white-space: nowrap;
  }
  ...
  .replication-panel .panel-header {
    background: #f8f6ff;
  }
  ...
  .replication-table .rep-dc { min-width: 210px; font-weight: 650; color: #24364b; }
  .replication-table .rep-partner { min-width: 210px; font-weight: 650; }
  .replication-table .rep-partition { min-width: 330px; font-family: Consolas, "Courier New", monospace; }
  .replication-table .rep-date { min-width: 155px; white-space: nowrap; }
  .replication-table .rep-result { min-width: 250px; }
  .replication-table .rep-failures { min-width: 115px; text-align: center; }

  .time-panel {
    border-left: 4px solid #c06c00;
  }
  ...
  .time-table .time-dc { min-width: 210px; font-weight: 650; color: #24364b; }
...
# Gera a seção visual de interfaces, endereçamento e DNS dos Domain Controllers.
$networkRows = foreach ($r in $ResultsArr) {
  ...
  if ($interfaces.Count -gt 0) {
    ...
    foreach ($nic in $interfaces) {
      ...
      $dnsHtml = [System.Net.WebUtility]::HtmlEncode([string]$nic.DNSServers)
      $dnsRegHtml = [System.Net.WebUtility]::HtmlEncode([string]$nic.DNSRegistration)

      if ($first) {
        "<tr><td class='network-dc' rowspan='$($interfaces.Count)'>$dcHtml</td><td class='network-count' rowspan='$($interfaces.Count)'>$countHtml</td><td class='network-interface'>$nameHtml</td><td class='network-addresses'>$addressesHtml</td><td class='network-dns'>$dnsHtml</td><td class='network-dns-registration'>$dnsRegHtml</td></tr>"
        $first = $false
      } else {
        "<tr><td class='network-interface'>$nameHtml</td><td class='network-addresses'>$addressesHtml</td><td class='network-dns'>$dnsHtml</td><td class='network-dns-registration'>$dnsRegHtml</td></tr>"
      }
    }
  } else {
    $summaryHtml = [System.Net.WebUtility]::HtmlEncode([string]$r.NetworkAddresses)
    $dnsSummaryHtml = [System.Net.WebUtility]::HtmlEncode([string]$r.NetworkDNSServers)
    "<tr><td class='network-dc'>$dcHtml</td><td class='network-count'>$countHtml</td><td colspan='2'>$summaryHtml</td><td colspan='2'>$dnsSummaryHtml</td></tr>"
  }
}
...
$networkSection = @"
  <section class='panel network-panel'>
    <div class='panel-header'>
      <h2 class='panel-title'>Interfaces, Endereçamento e DNS dos Domain Controllers</h2>
      <span class='legend-badge ok'>&#127760; Rede / DNS</span>
    </div>
    <div class='table-scroll'>
      <table class='network-table'>
        <thead>
          <tr>
            ...
            <th>Servidores DNS configurados</th>
            <th>Registro DNS</th>
          </tr>
        </thead>
        <tbody>
    ...
    <div class='network-note'>São contabilizadas interfaces com IP habilitado. Os endereços são apresentados com prefixo CIDR. A coluna de DNS respeita a ordem configurada na interface (DNS primário, secundário e demais servidores). Endereços de loopback são ignorados.</div>
  </section>
"@

# Gera a seção de replicação detalhada por parceiro e partição.
$replicationRelationshipTotal = 0
$replicationFailureTotal = 0
$replicationRows = foreach ($r in $ResultsArr) {
  $dcHtml = [System.Net.WebUtility]::HtmlEncode([string]$r.Identity)
  $details = @($r.ReplicationDetails)
  $replicationRelationshipTotal += $details.Count

  if ($details.Count -gt 0) {
    $first = $true
    foreach ($rep in $details) {
      if (-not $rep.IsSuccess) { $replicationFailureTotal++ }
      $partnerHtml = [System.Net.WebUtility]::HtmlEncode([string]$rep.Partner)
      $partitionHtml = [System.Net.WebUtility]::HtmlEncode([string]$rep.Partition)
      $attemptHtml = [System.Net.WebUtility]::HtmlEncode([string]$rep.LastAttempt)
      $successHtml = [System.Net.WebUtility]::HtmlEncode([string]$rep.LastSuccess)
      $resultCodeHtml = [System.Net.WebUtility]::HtmlEncode([string]$rep.ResultCode)
      $resultMessageHtml = [System.Net.WebUtility]::HtmlEncode([string]$rep.ResultMessage)
      $failuresHtml = [System.Net.WebUtility]::HtmlEncode([string]$rep.ConsecutiveFailures)
      $repClass = if ($rep.IsSuccess) { 'ok' } else { 'fail' }
      $repIcon = if ($rep.IsSuccess) { '&#10003;' } else { '&#10005;' }
      $resultHtml = "<span class='status-badge $repClass'>$repIcon $resultCodeHtml</span> $resultMessageHtml"

      if ($first) {
        "<tr><td class='rep-dc' rowspan='$($details.Count)'>$dcHtml</td><td class='rep-partner'>$partnerHtml</td><td class='rep-partition'>$partitionHtml</td><td class='rep-date'>$attemptHtml</td><td class='rep-date'>$successHtml</td><td class='rep-result'>$resultHtml</td><td class='rep-failures'>$failuresHtml</td></tr>"
        $first = $false
      } else {
        "<tr><td class='rep-partner'>$partnerHtml</td><td class='rep-partition'>$partitionHtml</td><td class='rep-date'>$attemptHtml</td><td class='rep-date'>$successHtml</td><td class='rep-result'>$resultHtml</td><td class='rep-failures'>$failuresHtml</td></tr>"
      }
    }
  } else {
    $summaryHtml = [System.Net.WebUtility]::HtmlEncode([string]$r.ReplicationSummary)
    "<tr><td class='rep-dc'>$dcHtml</td><td colspan='6'>$summaryHtml</td></tr>"
  }
}

$replicationSection = @"
  <section class='panel replication-panel'>
    <div class='panel-header'>
      <h2 class='panel-title'>Replicação Detalhada do Active Directory</h2>
      <div class='legend'>
        <span class='legend-badge ok'>$replicationRelationshipTotal relações</span>
        <span class='legend-badge fail'>$replicationFailureTotal com erro/indeterminado</span>
      </div>
    </div>
    <div class='table-scroll'>
      <table class='replication-table'>
        <thead>
          <tr>
            <th>Domain Controller (destino)</th>
            <th>Parceiro inbound</th>
            <th>Partição / Naming Context</th>
        ...
        <tbody>
          $($replicationRows -join "`n")
        </tbody>
      </table>
    </div>
    <div class='topology-note'>A tabela consulta os parceiros de replicação inbound de cada Domain Controller em todas as partições hospedadas. Código 0 representa a última tentativa concluída com sucesso.</div>
  </section>
"@

# Gera a seção de sincronismo de horário / Windows Time.
$timeRows = foreach ($r in $ResultsArr) {
  $dcHtml = [System.Net.WebUtility]::HtmlEncode([string]$r.Identity)
  $pdcHtml = if ($r.IsPdcEmulator) { 'Sim' } else { 'Não' }
  $serviceHtml = [System.Net.WebUtility]::HtmlEncode([string]$r.W32TimeService)
  $serverTimeHtml = [System.Net.WebUtility]::HtmlEncode([string]$r.ServerLocalTime)
  $sourceHtml = [System.Net.WebUtility]::HtmlEncode([string]$r.TimeSource)
  $lastSyncHtml = [System.Net.WebUtility]::HtmlEncode([string]$r.LastTimeSync)
  $stratumHtml = [System.Net.WebUtility]::HtmlEncode([string]$r.TimeStratum)
  $healthHtml = [System.Net.WebUtility]::HtmlEncode([string]$r.TimeHealth)
  $healthDetailHtml = [System.Net.WebUtility]::HtmlEncode([string]$r.TimeHealthDetail)
  $healthClass = if ($r.TimeHealth -eq 'OK') { 'ok' } else { 'warn' }
  $healthIcon = if ($r.TimeHealth -eq 'OK') { '&#10003;' } else { '&#9888;' }

  "<tr><td class='time-dc'>$dcHtml</td><td class='time-center'>$pdcHtml</td><td class='time-center'>$serviceHtml</td><td class='time-date'>$serverTimeHtml</td><td class='time-source'>$sourceHtml</td><td class='time-date'>$lastSyncHtml</td><td class='time-center'>$stratumHtml</td><td><span class='status-badge $healthClass'>$healthIcon $healthHtml</span><br/>$healthDetailHtml</td></tr>"
}

$timeSection = @"
  <section class='panel time-panel'>
    <div class='panel-header'>
      <h2 class='panel-title'>Sincronismo de Horário dos Domain Controllers</h2>
      <span class='legend-badge warn'>&#128337; Windows Time</span>
    </div>
    <div class='table-scroll'>
    ...
    <div class='network-note'>A fonte atual é obtida pelo Windows Time. Fontes locais como Local CMOS Clock / Free-running são destacadas como aviso. O campo PDC Emulator facilita a validação da hierarquia de tempo do domínio.</div>
  </section>
"@
